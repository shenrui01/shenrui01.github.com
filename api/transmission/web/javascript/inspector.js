    ...
    updateInfoPage = function () {
        ...
        if(torrents.length < 1)
            ...
            if( !haveUnverified && !leftUntilDone )
                ...
                str = fmt.size(haveVerified) + ' / ' + fmt.size(sizeWhenDone) + ' (' + str +'%), ' + fmt.size(haveUnverified) + ' 未校验';
        }
        ...
        if(torrents.length < 1)
            ...
            str = baseline ? '当前tracker为私有 -- 已禁用DHT和PEX' : '公开的';
            for(i=0; t=torrents[i]; ++i) {
        ...
        if(torrents.length < 1)
            ...
            if(mixed_creator && mixed_date)
                ...
                str = (new Date(date*1000)).toDateString() + '创建';
            else
                str = (new Date(date*1000)).toDateString() + ' ' + creator + '创建';
        }
        ...
        for (k=0; tor=torrents[k]; ++k)
        {
            ...
            for (i=0; peer=peers[i]; ++i) {
                ...
                html.push('<tr class="inspector_peer_entry ', parity, '">',
                       '<td>', (peer.isEncrypted ? '<div class="encrypted-peer-cell" title="加密连接">'
    ...
    getAnnounceState = function(tracker) {
        ...
        switch (tracker.announceState) {
            ...
            case Torrent._TrackerWaiting:
                ...
                s = '下一次通告间隔 ' + Transmission.fmt.timeInterval(timeUntilAnnounce);
                break;
    ...
    lastAnnounceStatus = function(tracker) {
        ...
        if (tracker.hasAnnounced) {
            ...
            if (tracker.lastAnnounceSucceeded) {
                lastAnnounce = [ lastAnnounceTime, ' (获得 ',  Transmission.fmt.countString('peer','个节点',tracker.lastAnnouncePeerCount), ')' ];
            } else {
